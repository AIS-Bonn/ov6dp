...
class OV6DPProcessorConfig:
    ...
    vocab: str="banana. tomato can. apple."
    prefix: str="image"
...
class OV6DPProcessor(ImageProcessor):
    ...
    def process_images(self, images: np.ndarray, info: MetaInfoSizeBase) -> np.ndarray | Tuple[np.ndarray, Any]:
        ...
        if key == ord('s'):  # Save image if 's' is pressed
            image_path = os.path.join(self.save_dir, f"{self.prefix}_depth_{self.image_count}.npy")
            np.save(image_path, depth_image)

            image_path = os.path.join(self.save_dir, f"{self.prefix}_rgb_{self.image_count}.jpg")
...
def parse_opt():
    ...
    parser.add_argument("--provider_name", default="AzureKinectPointCloudProvider", type=str,
                        help="name of the image provider")
    parser.add_argument("--agent_name", default="ArmarDE", type=str,
                        help="name of the robot/agent")
    parser.add_argument("--frame_name", default="AzureKinect_RGB", type=str,
